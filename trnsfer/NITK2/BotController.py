...
class Bot(object):
    ...
    def UpdateProperties():
        ...
        if(len(backCheckPointList) <=0 or len(frontCheckPointList)  <= 0):
            print "Failed to Capture bot position !!! >>>>>>>>>>>>>> " + str(len(backCheckPointList)) + " " + str(len(frontCheckPointList))
    ...
    @staticmethod
    def Traverse(ListOfResources, ListOfObstacles = None,ListOfChkpt=None): #ListOfChkpt=None
        print "Townhall center is:" + str(Frame.townHall.center.toString())
        for target in ListOfResources:
            ...
            if target.path != None:
                for node in target.path:
                    ...
                    # for chkpt in  ListOfChkpt:
                    #     if Point.chkptinRange(Bot.position, chkpt):
                    #         print 'Reached checkpoint  <<<<<<<<<<<<<<<< '
                    #         Bot.Stop()
                    #         sleep(5)
                    #         ListOfChkpt.remove(chkpt)
                    ...
                    if Point.inRange(Bot.position, node):
                        ...
                        while not Point.inRange(Bot.position, node):
                            for chkpt in  ListOfChkpt:
                                if Point.chkptinRange(Bot.position, chkpt.center):
                                    print 'Reached checkpoint  <<<<<<<<<<<<<<<< '
                                    Bot.Stop()
                                    sleep(5)
                                    ListOfChkpt.remove(chkpt)
