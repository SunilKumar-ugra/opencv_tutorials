...
class Grid(object):
    ...
    @staticmethod
    def find_chkpt(chkpt_checkPoints, totalchkptPoints=None):
        all_chkpt=[]

        totalChkptPoints = 0
        chkpt_range = Config.chkptRange #round(5 * Config.mapRatio)

        for currentPoint in chkpt_checkPoints: #takes Checkpoint as input

            point1 = (currentPoint.gridCenter.x - chkpt_range, currentPoint.gridCenter.y - chkpt_range)
            point2 = (currentPoint.gridCenter.x + chkpt_range, currentPoint.gridCenter.y - chkpt_range)
            point3 = (currentPoint.gridCenter.x - chkpt_range, currentPoint.gridCenter.y + chkpt_range)
            point4 = (currentPoint.gridCenter.x + chkpt_range, currentPoint.gridCenter.y + chkpt_range)

            Config.chkptBoundingPointList.append((point2, point1))
            Config.chkptBoundingPointList.append((point3, point1))
            Config.chkptBoundingPointList.append((point3, point4))
            Config.chkptBoundingPointList.append((point2, point4))

            corner_points = [point1, point2, point3, point4]

            for p in range(corner_points[0][0], corner_points[1][0] + 1): #take X coordinate
                    q = corner_points[0][1] #Y is constant
                    all_chkpt.append((p,q))
                    totalchkptPoints += 1
